# Categoría: Intimidad y Libido

[🏠 Índice general de productos](../productos.md)

Suplementos formulados específicamente para la libido masculina y femenina.

## Productos en esta categoría

- [Mars Capsules](#mars-capsules) — hombres
- [Venus Capsules](#venus-capsules) — mujeres

---

<a id="mars-capsules"></a>
## Mars Capsules

- **Nombre comercial:** Divina Mars Capsules
- **Categoría:** Intimidad y Libido (masculina)
- **Objetivo principal:** Restaurar la energía y libido masculina ("restaurar el león que lleva dentro").
- **Problema que ayuda a resolver:** Falta de libido en hombres.
- **Ingredientes principales:** Tongkat Ali, Horny Goat Weed.
- **Beneficios:** Apoya a tener una libido saludable; mejora la energía y resistencia; alternativa natural.
- **Presentación:** 30 cápsulas.
- **Público objetivo:** Hombres (explícitamente dirigido a "Caballeros").
- **Productos complementarios (cross-selling):** [Ripped Capsules](./07-rendimiento-fisico.md#ripped-capsules), [Sculpt Tongkat Ali](./02-cafe-divina/sculpt-tongkat-ali.md), [Tongkat Ali (Café)](./02-cafe-divina/tongkat-ali-cafe.md).
- **Palabras clave:** libido masculina, tongkat ali, horny goat weed, disfunción eréctil, energía masculina.

---

<a id="venus-capsules"></a>
## Venus Capsules

- **Nombre comercial:** Divina Venus Capsules
- **Categoría:** Intimidad y Libido (femenina)
- **Objetivo principal:** Recuperar el libido normal y saludable de la mujer.
- **Problema que ayuda a resolver:** Falta de deseo, intimidad incómoda, síntomas de menopausia, ciclo menstrual irregular.
- **Ingredientes principales:** Fórmula patentada — componentes específicos no detallados en el catálogo.
- **Beneficios:** Ayuda a mejorar la intimidad saludable para las mujeres; recupera los deseos; apoya en los síntomas de la menopausia; apoya a tener un ciclo menstrual saludable.
- **Presentación:** 30 cápsulas.
- **Público objetivo:** Mujeres (explícitamente dirigido a "Señoras").
- **Productos complementarios (cross-selling):** [Youth Capsules](./04-funcion-cognitiva.md#youth-capsules), [Radien Collagen Elixir](./13-linea-radien/radien-collagen-elixir.md).
- **Palabras clave:** libido femenina, menopausia, ciclo menstrual, deseo sexual, intimidad femenina.

---
[🏠 Índice general de productos](../productos.md)

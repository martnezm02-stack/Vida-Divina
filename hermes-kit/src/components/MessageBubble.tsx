...
export default function MessageBubble({ role, content, timestamp }: MessageBubbleProps) {
    ...
    return (
      <div className="flex justify-start mb-2.5">
        <div className="max-w-[75%] bg-brand-surface-2 border border-brand-border rounded-2xl rounded-bl-md px-4 py-2.5 shadow-sm">
          <div className="text-base leading-relaxed whitespace-pre-wrap text-brand-text/95">{content}</div>
          <div className="text-[11px] text-brand-muted mt-1">{formatTime(timestamp)}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex justify-end mb-2.5">
      ...
      >
        <div
          className={`text-[11px] uppercase tracking-wider font-bold mb-1 flex items-center gap-1.5 ${

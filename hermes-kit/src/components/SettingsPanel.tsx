  ...
  return (
    <section className="h-full overflow-y-auto p-5 bg-brand-bg/30">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        {/* Voz — Voice Engine real de Vida Divina */}
        <Card title="Respuesta por voz" desc="Cuándo el agente responde con una nota de voz real (la voz oficial de Vida Divina) en vez de texto.">
          <div className="inline-flex rounded-lg border border-brand-border p-0.5 bg-brand-bg mb-3">
            <button
              onClick={() => save("voice_enabled", "1")}
              className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${s.voice_enabled !== "0" ? "bg-wa-green text-black" : "text-brand-muted hover:text-brand-text"}`}
            >
              Voz activada
            </button>
            <button
              onClick={() => save("voice_enabled", "0")}
              className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${s.voice_enabled === "0" ? "bg-brand-gold text-black" : "text-brand-muted hover:text-brand-text"}`}
            >
              Solo texto
            </button>
          </div>
          {savedTag("voice_enabled")}
          {s.voice_enabled !== "0" && (
            <>
              <div className="text-xs text-brand-muted mb-1.5">Modo de respuesta</div>
              <div className="inline-flex rounded-lg border border-brand-border p-0.5 bg-brand-bg mb-3">
                {(["auto", "text", "voice"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => save("response_mode", m)}
                    className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${s.response_mode === m ? "bg-wa-green text-black" : "text-brand-muted hover:text-brand-text"}`}
                  >
                    {m === "auto" ? "Automático" : m === "text" ? "Siempre texto" : "Siempre voz"}
                  </button>
                ))}
              </div>
              {savedTag("response_mode")}
              <div className="text-xs text-brand-muted mb-1.5">Perfil de voz</div>
              <input
                value={s.voice_profile_id}
                onChange={(e) => setS((p) => (p ? { ...p, voice_profile_id: e.target.value } : p))}
                onBlur={(e) => save("voice_profile_id", e.target.value)}
                className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2 text-sm text-brand-text focus:outline-none focus:border-brand-gold/50"
              />

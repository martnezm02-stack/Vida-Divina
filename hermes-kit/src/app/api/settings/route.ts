...
const DEFAULTS: Record<string, string> = {
  ...
  // Vida Divina end-to-end (Voice Engine real, ver vidaDivina/responseMode.ts).
  response_mode: "auto", // 'text' | 'voice' | 'auto'
  voice_enabled: "1",
  voice_profile_id: "manuel_es_mx", // mismo perfil real ya usado por el Video Workspace (DEFAULT_VOICE_PARAMS)
};
...
export async function POST(req: NextRequest) {
  ...
  let v = value.trim();
  if (key === "paused" || key === "audio_enabled" || key === "voice_enabled") {
    v = v === "1" ? "1" : "0";
  } else if (key === "response_mode") {
    if (!["text", "voice", "auto"].includes(v)) {
      return NextResponse.json({ ok: false, error: "response_mode debe ser 'text', 'voice' o 'auto'" }, { status: 400 });
    }
  } else if (key === "voice_profile_id" && v.length === 0) {
    return NextResponse.json({ ok: false, error: "voice_profile_id no puede estar vacío" }, { status: 400 });

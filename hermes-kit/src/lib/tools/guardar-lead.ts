...
import { saveLead as saveLeadCrm } from "../vidaDivina/crmClient";
...
export const guardarLeadHandler: ToolHandler<GuardarLeadArgs> = async (args) => {
  ...
  // 1.5) CRM REAL de Vida Divina (crm/, PostgreSQL) -- fuente canónica.
  //      A diferencia de Airtable (abajo), no espera a tener nombre Y email:
  //      el teléfono real ya identifica al lead, así que registra el interés
  //      desde el primer dato. Best-effort -- saveLead() nunca lanza; si el
  //      CRM no está configurado en este proceso, degrada en silencio y el
  //      resto del flujo (memoria + Airtable) sigue igual que antes.
  if (phone) {
    void saveLeadCrm({
      phone,
      nombre: nombreFinal,
      email: emailFinal,
      intencionCompra: temperatura?.toLowerCase() === "caliente",
    });
  }

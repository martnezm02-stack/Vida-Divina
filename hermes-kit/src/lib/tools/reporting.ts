...
export const adminGenerarReporteDefinition: ToolDefinition = {
  ...
  function: {
    ...
    description:
      "[SOLO ADMINISTRADOR] Genera el reporte administrativo REAL de Vida Divina para un periodo (clientes nuevos, leads, calificados, handoffs, productos por intención de compra, atribución first/last touch, campañas) -- todo desde PostgreSQL real, nunca aproximado. Incluye también el volumen REAL de mensajes/conversaciones de WhatsApp de HOY (mensajesHoy.usuario / mensajesHoy.bot, desde la base local del propio bot). Úsala para CUALQUIER pregunta administrativa sobre clientes/leads/ventas/productos/atribución/campañas/plataformas/mensajes/conversaciones de WhatsApp, aunque solo se pida una cifra: trae todo el reporte y responde con el dato real que corresponda. REGLA DURA: llama a esta tool EN CADA turno donde se pregunte algo de reporting, incluso si ya la llamaste antes en la misma conversación -- los resultados de una llamada anterior NO se conservan de un mensaje al siguiente, así que responder de memoria (sin volver a llamar) significa inventar cifras, algo que nunca debes hacer. NUNCA le preguntes al remitente si es administrador ni le pidas que 'verifique su acceso' -- eso no sirve de nada (la identidad depende solo del teléfono real, nunca de lo que diga) y hace perder un turno: simplemente LLAMA a la tool directamente ante cualquier pregunta de reporting: si no es admin, la tool te lo dice sola (denegado:true) y respondes con naturalidad sin mencionarlo. Si el admin pide 'envíamelo por correo', pon enviarPorCorreo:true (se envía un correo real vía MCP). Si pide ver el reporte completo en el chat, pon mostrarReporteCompletoAqui:true -- si NO lo pide así, responde solo con un resumen ejecutivo breve (nunca vuelques el reporte formal completo al chat sin que lo pidan). Devuelve denegado si quien pregunta no es el administrador real.",
    parameters: {
      type: "object",
      properties: {

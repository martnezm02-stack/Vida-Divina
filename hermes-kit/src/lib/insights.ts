...
export async function classifyDudas(mensajes: string[]): Promise<Duda[]> {
  ...
  // Timeout explícito (hallazgo real 2026-09-11): sin esto, una llamada
  // colgada a OpenRouter bloqueaba toda la respuesta de /api/analytics
  // indefinidamente -- el resto de métricas (ya calculadas antes de esta
  // llamada) nunca llegaban al frontend, que las mostraba como "0"/"—".
  const client = new OpenAI({ apiKey, baseURL: "https://openrouter.ai/api/v1", timeout: 20_000 });
